            ...
            try {
                ...
                const countAvailable = (type) => {
                    ...
                    if (targetType === 'custom module') {
                        const uniqueCM = new Set((schedules || []).filter(s => s.custom_module_code && s.custom_module_code !== '-' && s.custom_module_code !== '').map(s => (s.custom_module_code || '').trim()));
                        count = uniqueCM.size;
                    } else if (targetType === 'marrow gt') {
                        const uniqueGT = new Set((schedules || []).filter(s => s.marrow_gt && s.marrow_gt !== '-' && s.marrow_gt !== '').map(s => (s.marrow_gt || '').trim()));
                        count = uniqueGT.size;
